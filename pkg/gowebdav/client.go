...
// WriteStream writes a stream without buffering the entire content.
func (c *Client) WriteStream(path string, stream io.Reader, _ os.FileMode) (err error) {
	// Создаем родительские коллекции (директории)
	err = c.createParentCollection(path)
	if err != nil {
		return err
	}

	// Определяем размер контента, если поток поддерживает Seek
	...
	if seeker, ok := stream.(io.Seeker); ok {
		...
		// Если поток не поддерживает Seek, отправляем данные без указания Content-Length
		contentLength = -1
	}

	// Выполняем PUT-запрос с потоком данных
